...
/** Loaded-only reconciliation for BetterChests drawer persistence. */
                        ...
                        switch (inspection.status()) {
                            ...
                            case LEGACY_RECOVERABLE -> {
                                ...
                                addDetail(details, block,
                                        "recoverable Dev-16 drawer state: " + inspection.data().count() + " stored item(s)");
                            ...
                            case LEGACY_UNRESOLVED -> {
                                issues++;
                                addDetail(details, block,
                                        "legacy/uninitialized drawer needs manual verification: " + inspection.detail());
                            }
                            case CORRUPT_MODERN -> {
                                issues++;
                                addDetail(details, block,
                                        "corrupt/inconsistent v2 drawer left untouched: " + inspection.detail());
                            }
                        }
                    } catch (RuntimeException | LinkageError exception) {
                        failures++;
                        BetterChests.INSTANCE.getLogger().warning(
                                "Doctor could not inspect BetterChests drawer at " + format(block) + ": "
                                        + exception.getMessage());
                        addDetail(details, block, "inspection failed safely; drawer left untouched");
                    }
                }
            }
        }
